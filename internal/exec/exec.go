...
func DownloadTwitchLiveVideo(ctx context.Context, video ent.Vod, channel ent.Channel, startChat chan bool) error {
	...
	if !proxyFound {
		...
		masterPlaylist, err = tc.GetStream(context.Background(), channel.Name)
		if err != nil {
	...
	if video.VideoHlsPath == "" {
		...
		ffmpegArgs = append(ffmpegArgs, []string{"-start_number", "0", "-hls_time", "10", "-hls_list_size", "0", "-hls_segment_filename", fmt.Sprintf("%s/%s_segment%s.ts", video.VideoHlsPath, video.ExtID, "%d"), "-f", "hls"}...)
	...
	ffmpegArgs = append(ffmpegArgs, video.VideoPath)
